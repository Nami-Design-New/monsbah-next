...
/**
 * Products sitemap route: /[locale]/products/sitemap0.xml
 * Handles: /sa-ar/products/sitemap0.xml, /kw-ar/products/sitemap1.xml, etc.
 * 
 * Route structure: /[country-locale]/products/[index]
 * The [index] parameter will receive values like "sitemap0.xml", "sitemap1.xml"
 */
export async function GET(request, { params }) {
  try {
    const resolvedParams = await params;
    const locale = resolvedParams["country-locale"];
    let indexParam = resolvedParams["index"]; // e.g., "sitemap0.xml"
    
    console.log(`[Products Sitemap] RAW params:`, JSON.stringify(resolvedParams));
    console.log(`[Products Sitemap] Locale: ${locale}, Index param: "${indexParam}"`);
    
    // Remove .xml extension if present
    indexParam = indexParam.replace(/\.xml$/, '');
    
    console.log(`[Products Sitemap] After cleanup: "${indexParam}"`);
    
    // Extract index from filename (sitemap0 -> 0, sitemap5 -> 5)
    const match = indexParam.match(/^sitemap(\d+)$/);
    if (!match) {
      console.log(`[Products Sitemap] Invalid format: "${indexParam}", expected pattern: sitemap{number}`);
      return new Response(`Invalid sitemap filename: ${indexParam}`, { status: 400 });
    }
